...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to clone"),
    parentId: import_zod.z.string().describe("The ID of the parent node to place the clone in"),
    x: import_zod.z.number().optional().describe("X position within the new parent"),
    y: import_zod.z.number().optional().describe("Y position within the new parent")
  },
...
server.tool(
  ...
  "Move a node to a different parent node in Figma (changes the layer hierarchy)",
  {
    ...
    index: import_zod.z.number().int().min(0).optional().describe("Optional index position within the new parent's children")
  },
...
server.tool(
  ...
  {
    times: import_zod.z.number().int().min(1).max(50).optional().describe("Number of undo steps (default: 1, max: 50)")
  },
  async ({ times }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().optional().describe("The root node ID to search (default: current page)")
  },
  async ({ nodeId }) => {
    try {
      ...
      const nodeList = typedResult.hiddenNodes.map((n) => `- ${n.name} (${n.type}, ID: ${n.id}, parent: ${n.parentName})`).join("\n");
      return {
        content: [
          {
            type: "text",
...
var exportNodeToFileSchema = {
  nodeId: import_zod.z.string().describe("The ID of the node to export"),
  filePath: import_zod.z.string().describe("Full local file path to save the exported image (e.g. ~/Downloads/icon.png). Path must be within the user's home directory. Override allowed directories via FIGMA_EXPORT_ALLOWED_DIRS env var."),
...
server.tool(
  ...
  {
    outputDir: import_zod.z.string().describe("Local directory path to save exported images. Must be within the user's home directory."),
